...
//    String[] type=new String[]{"物流发货单","司机接货单"};
    ...
    private String type;
    private List<AuditOrderEntity> auditOrderEntityList;
    private List<AuditOrderEntity1> auditOrderEntityList1;
...
//                        .isDialog(true)//是否显示为对话框样式
    ...
    private void initwuliufahuo() {
        ...
        PileApi.instance.searchOrderByParamSJ(data)
                    ...
                    public void onNext(@NonNull ResponseBody responseBody) {
                        try {
                            ...
                            if (body.length() > 10) {
                                Gson gson1 = new Gson();
                                if (ed_type.getText().toString().trim().equals("同城小件单")){
                                    auditOrderEntityList = gson1.fromJson(body, new TypeToken<List<AuditOrderEntity>>() {
                                    }.getType());
                                }else if (ed_type.getText().toString().trim().equals("同城搬家单")){
                                    auditOrderEntityList1 = gson1.fromJson(body, new TypeToken<List<AuditOrderEntity1>>() {
                                    }.getType());
                                }else {
                                    auditOrderEntityList2 = gson1.fromJson(body, new TypeToken<List<KuaiYunEntity>>() {
                                    }.getType());
                                }
...
//                                intent.putExtra("shuju", (Serializable) huoDiSearchList);
                                if (ed_type.getText().toString().trim().equals("同城小件单")) {
                                    ...
                                    intent.putExtra("shuju", (Serializable) auditOrderEntityList);
                                } else if (ed_type.getText().toString().trim().equals("同城搬家单")) {
                                    intent.putExtra("type", "1");
                                    intent.putExtra("shuju1", (Serializable) auditOrderEntityList1);
                                } else {
                                    intent.putExtra("type", "2");
                                    intent.putExtra("shuju2", (Serializable) auditOrderEntityList2);
                                }
